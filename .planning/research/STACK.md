# Technology Stack — PostmanClone

**Project:** PostmanClone (Postman-like desktop API client for Java Spring developers)
**Dimension:** Stack (specific libraries, versions, rationale)
**Researched:** 2026-06-03
**Mode:** Ecosystem / prescriptive stack for greenfield
**Overall confidence:** HIGH (every primary choice verified against Context7 or official sources; precise 2026 versions confirmed)
...
**One-liner:** **Electron 42 + React 19 + Vite 8 + TypeScript** for the UI, talking via Electron IPC to a **Node 24** main process, which supervises a long-lived **Java 21** subprocess running **JavaParser + HikariCP** over **JSON-RPC 2.0 on stdio**.
...
ARCHITECTURE.md already recommends Electron. **We confirm it.** The decisive reasons for this project specifically:

1. **Mature Java subprocess control.** `child_process.spawn` with piped stdio is the textbook path for a long-lived JVM helper. Tauri's `Command` / `tauri-plugin-shell` is younger and has fewer production deployments for this exact shape of integration.
2. **Universal JDBC.** JDBC drivers for PostgreSQL/MySQL/Oracle/H2 are Java-only. Spawning a JVM is the *only* realistic way to cover all four v1 drivers with first-class support. Tauri would force a Rust SQL stack (`sqlx` / `tokio-postgres` / `mysql_async` / `rusqlite`) with patchier Oracle support and no good H2 client.
3. **Pre-warmed JVM helper.** A 2-5s one-time hit then ~50ms per call beats Tauri's process model for the "open project → scan → get endpoints" loop. ARCHITECTURE.md §3.1's table is correct.
4. **No `tauri-plugin-stronghold` needed.** Electron's built-in `safeStorage` (DPAPI / Keychain / libsecret) is mature, async, OS-native, and documented to "protect content from other users and other apps running in the same userspace" on Windows and macOS. No need for a third-party secrets plugin.
5. **Mature renderer ecosystem.** React 19 + Vite 8 + `@vitejs/plugin-react` v6 (Oxc-based) is a battle-tested combination. Tauri's frontend tooling story is improving but not at parity.
6. **CORS avoidance by design (PITFALLS C-7).** When HTTP lives in the main process via undici, it is not subject to browser CORS. Renderer's `fetch` would be. Architecture designed from day one to avoid a class of user pain.
...
**Rejected by PROJECT.md Constraint:** "Must run as a desktop application (not just a web app) so it can read the local Spring project's source/classpath and connect to local databases without server-side proxies."
...
**Recommendation: `electron-vite` for dev/HMR + `electron-builder` for packaging.**
...
Use `electron-builder` for the actual installer artifacts (`.exe`/`.dmg`/`.AppImage`/`.deb`). `electron-vite` orchestrates dev. `electron-forge` is more opinionated; mixing it with Vite is doable but more wiring.
...
All versions verified against Context7 (Vite 8.0.0 stable 2026-03-12, React 19.2.0 stable, Zustand 5.0.12, TanStack Query 5.90.3, Zod 4.0.1, react-virtuoso 4.6.2).
...
**Why undici, not the alternatives:**
...
**Important CORS note (PITFALLS C-7):** All HTTP from the desktop app to user-configured targets must come from the **main process via undici**, not from the renderer. Renderer `fetch` is subject to browser CORS, which would require the user's Spring app to whitelist the desktop app's origin. By going through main, we sidestep CORS entirely. ARCHITECTURE.md §3.2 has this correct.

**`Origin` header:** When using undici, deliberately set `Origin` to match the target server's host (e.g., `Origin: http://localhost:8080`) so backend CORS configs that key off the Origin header don't 403 us. The Electron main process is not a browser and isn't subject to CORS itself, but the *server* may still inspect `Origin`.
...
**Why electron-log:** Purpose-built for Electron's `userData`/log path. Handles file rotation, async writes, console capture. Better than `winston` or `pino` for desktop apps where you don't need a remote transport.
...
**The single most important decision in this section** (PITFALLS C-6).
...
**Always use the async API (`encryptStringAsync` / `decryptStringAsync`):** per Electron docs: "The async API is non-blocking, supports key rotation, and handles temporary unavailability gracefully. The synchronous API may be deprecated in a future version of Electron."

**Usage pattern:** Encrypt DB connection passwords at rest in `db-connections/<id>.json`; decrypt only when opening the JDBC pool. Never log the ciphertext as "encrypted password"; never log plaintext.
...
**Why Java 21 over Java 17 for the helper:**
...
PITFALLS C-2 ("JavaParser Symbol Resolution Falls Over on Real Spring Projects") is the single biggest risk. **We confirm the recommendation in ARCHITECTURE.md** and add specific mitigations driven by the pitfall:
...
**Important version note:** Use **1.6**, not 1.7. The 1.7 SNAPSHOT on the master branch has been there for years and is not a stable release.

**Wire format:** Newline-delimited JSON-RPC 2.0 (LSP-style). One JSON-RPC message per line on stdout; stderr is reserved for logs (the main process forwards stderr to `logs/helper.log`).

**Why not roll our own JSON-RPC in Java?** The framing is trivial; the error handling and request ID correlation are not. `jsonrpc4j` saves a week of work and is a stable, single-class-file dependency.

**Why not on the Node side also use a library?** `undici` already gives us `fetch`, and the JSON-RPC client is ~80 lines of code (newline framing, request ID map, response matching). Adding a library to the main process for one side of a single IPC channel is overkill.
...
The helper is primarily stdio JSON-RPC, but we still need a tiny CLI to surface diagnostic info (`--version`, `--health`, `--scan-dry-run <path>` for debugging). **picocli 4.7+** is the de-facto standard for Java CLIs and adds ~600 KB to the jar. A minimal `Main` class with `@Command` annotations is enough.
...
**Driver bundling strategy:** All four drivers go into the helper's fat jar (built with Gradle's `shadowJar` or Maven's `maven-shade-plugin`). The user does **not** need to provide driver jars (PITFALLS decision per ARCHITECTURE.md §7.4 step 2).

**DB type normalization** (PITFALLS C-4): The helper needs a per-driver normalizer for `json`/`jsonb` columns (PostgreSQL `PGobject`), `String` JSON (H2, MySQL), and `SQLXML` (Oracle JSON in some versions). This is application logic, not a library choice — it lives in the helper's `DbRowToJson` class.
...
Every write goes to `<file>.tmp` → `fsync` → `rename`. No partial JSON on crash. Use `fs.promises.writeFile` + `fs.promises.rename`, or `proper-lockfile` if concurrent-write concerns emerge (unlikely for single-user desktop).

### Cloud-synced folder detection (PITFALLS m-5)

On first run, detect if the data dir resolves to a known-synced path (Dropbox, OneDrive, iCloud Drive) and warn. We do NOT auto-redirect; we surface the choice to the user.
...
We adopt the **Postman Collection Format v2.1** as our collection file format. Reasons:
...
**Validation:** A Zod schema (or hand-written TS interface) mirrors the v2.1 spec for what we import; we normalize into our internal model on import and back to v2.1 on export. Anything we can't parse, we report (PITFALLS M-10).

**Round-trip test corpus:** Maintain a fixture of 20+ real Postman collections (publicly shared ones on GitHub) and assert round-trip equality in CI.

**Version detection:** A v2.1 collection has a top-level `info.schema = "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"`. v2.0 collections have no `schema` field; v1.x is a different shape entirely. We accept v2.0 and v2.1; reject v1.x with a friendly "save in Postman as v2.0 first" error.
...
The body generator walks the JavaParser `ResolvedType` tree directly and emits a *placeholder JSON* with cycle detection (PITFALLS C-1, C-3). It does **not** need a full JSON-Schema generator because the consumer is the user's request body, not a validator.

**When we do want JSON Schema** (for chain-step schema validation or future "this body matches the DTO" assertions), use:
...
For the renderer, the same applies in reverse: we display placeholder JSON to the user; if the user wants schema validation in the future, we ship the generated schema from the helper and validate client-side with `ajv` (8.x, the standard).
...
dependencies {
  ...
  implementation("com.fasterxml.jackson.core:jackson-annotations:2.21.2")
  ...
  // Logging (java.util.logging is built-in; we use it directly)
}
```

(Note: `ojdbc11` and `mysql-connector-j` are not on Maven Central by default; they require adding Oracle's repo or `mavenLocal()` for some distributions. Document this in the helper's `README`.)
...
Phase 0 (skeleton):
  ...
  - Main process: undici + safeStorage + electron-log + electron-store
  - Preload: contextBridge with typed `window.api.*`
  - Storage service: atomic JSON writes

Phase 1 (HTTP + collections):
  - Add: postman-collection v2.1 schema types (Zod)
  - Main: undici fetch with per-request dispatcher + variable resolver
  - Renderer: Monaco body editor, react-virtuoso for history, react-virtuoso for response

Phase 2 (Spring scanner):
  - Helper scaffold: JDK 21 + Gradle 8 shadowJar
  - Add to helper: JavaParser 3.28.1, ASM 9.7, Jackson 2.21.2, jsonrpc4j 1.6
  - Add to main: chokidar 4 for project file watching
  - Test corpus: 10+ real Spring projects for symbol-solver coverage

Phase 3 (DTO body gen):
  - Helper: hand-rolled schema walker + cycle detection
  - Renderer: Monaco body editor in DTO-mode

Phase 4 (DB body gen):
  - Helper: HikariCP 6 + 4 JDBC drivers (PostgreSQL 42.7.11, MySQL 9.x, Oracle 23.x, H2 2.3.x)
  - Main: connection management UI + safeStorage-encrypted credentials

Phase 5 (Chains):
  - Renderer: JSONata 1.8 for field-path expressions + Monaco for step results
  - Main: chain runner, per-step retry/timeout policy
```
...
These are **not** blockers for the stack — they're decisions that should be revisited when the corresponding phase lands.
...
All links below were accessed 2026-06-03. Confidence levels are per the table in §11.
...
**Electron 42 + React 19 + Vite 8 + TypeScript** for the UI, talking via Electron IPC to a **Node 24** main process, which supervises a long-lived **Java 21** subprocess running **JavaParser 3.28.1 + HikariCP 6** over **JSON-RPC 2.0 on stdio**, with secrets in **Electron `safeStorage`** and persistence in **JSON files in `userData`**.
