...
- **D-04:** **Re-run from step 1 to N (always fresh).** When the user re-runs a single step, all steps from 1 to N are re-executed with fresh HTTP requests. No cached responses — always current data. All step results are replaced.
